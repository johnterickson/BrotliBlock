...
namespace BrotliBlockLib
{
    ...
    public sealed partial class BrotliBlockStream : Stream
    {
        ...
        private bool _needs_start_block;
        private bool _needs_end_block;
...
#endif
        {
            ...
            while (!TryDecompress(buffer, out bytesWritten))
            {
                ...
                if (bytesRead <= 0 && _needs_end_block)
                {
                    EndBlock.CopyTo(new Span<byte>(_buffer, _bufferCount, _buffer.Length - _bufferCount));
                    bytesRead = EndBlock.Length;
                    _needs_end_block = false;
                }
        ...
        private bool TryDecompress(Span<byte> destination, out int bytesWritten)
        {
            ...
            if (_needs_start_block)
            {
                readBuffer = new ReadOnlySpan<byte>(GetStartBlock(_window_bits));
            }
            else
            {
                readBuffer = new ReadOnlySpan<byte>(_buffer, _bufferOffset, _bufferCount);
            }

            OperationStatus lastResult = _decoder.Decompress(readBuffer, destination, out int bytesConsumed, out bytesWritten);
            if (lastResult == OperationStatus.InvalidData)
            {
                throw new InvalidOperationException("SR.BrotliStream_Decompress_InvalidData");
            }

            if (!_needs_start_block && bytesConsumed != 0)
            {
                _bufferOffset += bytesConsumed;
                _bufferCount -= bytesConsumed;
            }

            _needs_start_block = false;
