...
namespace BrotliBlockLib
{
    ...
    internal partial struct BrotliEncoder : IDisposable
    {
        ...
        internal void SetParameter(BrotliEncoderParameter parameter, uint value)
        {
            if (Interop.Brotli.BrotliEncoderSetParameter(_state, parameter, value) == Interop.BOOL.FALSE)
            {
                throw new InvalidOperationException("SR.BrotliEncoder_InvalidSetParameter, Quality");
            }
        }
        ...
        internal void SetQuality(int quality)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(quality), "SR.BrotliEncoder_Quality, quality, 0, BrotliUtils.Quality_Max");
            }
        ...
        internal OperationStatus Flush(Span<byte> destination, out int bytesWritten) => Compress(ReadOnlySpan<byte>.Empty, destination, out _, out bytesWritten, BrotliEncoderOperation.Flush);
